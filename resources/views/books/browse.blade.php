...
@push('scripts')
    <script>
        ...
        document.addEventListener('click', function(e) {
            if (e.target.classList.contains('dropdown-option')) {
                ...
                if (dropdownEl) {
                    ...
                    .then(data => {
                        if (data.success) {
                            showBrowseToast('✔ Status updated to "' + statusText + '"!');
                        } else {
        ...
        function showBrowseToast(msg, bg = '#adbda3') {
            let toast = document.getElementById('browse-toast');
            if (!toast) {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        if (data.action === 'added') {
                            showBrowseToast('✔ Added to Favorites!');
                            checkbox.checked = true;
                        } else {
                            showBrowseToast('✔ Removed from Favorites!');
            ...
            if (option) {
                ...
                if (dropdownEl) {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            showBrowseToast('✔ Added to "' + shelfName + '"!');
                            const label = dropdownEl.querySelector('.bookshelf-label');
                            if (label) label.textContent = shelfName;
                        } else {
                            showBrowseToast(data.message || 'Could not add to shelf.', '#d32f2f');
                        }
                    })
                    .catch(err => {
                        console.error(err);
                        showBrowseToast('An error occurred.', '#d32f2f');
                    });
                }
            }
        });
            ...
            if (dropdownEl) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        showBrowseToast('✔ Shelf created & book added!');
                    } else {
                        showBrowseToast(data.message || 'Could not create shelf.', '#d32f2f');
                    }
                })
                .catch(err => {
                    console.error(err);
